    ...
    <div className="flex flex-col gap-8 pb-12">
                ...
                <GlassCard key={project._id} className="p-6 border-sky-400/25 shadow-xl flex flex-col gap-5">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-sky-400/15">
                    <div className="flex items-center gap-3">
                      <div className="p-2.5 rounded-xl bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-400/20">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex flex-col items-end">
                        <span className="text-[10px] text-slate-700 dark:text-slate-300 uppercase font-bold">Overall Site Progress</span>
                        <span className="text-base font-bold text-sky-600 dark:text-sky-400">{projectProgress}%</span>
                      </div>
                      <Button
                        variant="glass"
                        size="sm"
                        ...
                        >
                          <div>
                            <div className="flex items-start justify-between gap-2 mb-2">
                              <span className="text-xs font-bold text-slate-950 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-300">
                                {room.name}
                              </span>
                              <span
                                className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${
                                  room.executionStatus === 'Completed'
                                    ...
                                    : 'bg-slate-200 dark:bg-charcoal-800 text-slate-800 dark:text-slate-200 border-slate-300 dark:border-slate-700'
                                }`}
                              >
                            ...
                            <div className="flex flex-col gap-1 mb-3">
                              <div className="flex items-center justify-between text-[10px]">
                                <span className="text-slate-700 dark:text-slate-300 font-medium">Construction</span>
                                <span className="font-bold text-amber-600 dark:text-amber-400">{roomProgress}%</span>
                              </div>
                              <ProgressBar progress={roomProgress} size="sm" variant={roomProgress === 100 ? 'emerald' : 'amber'} />
                            </div>

                            <div className="text-[10px] text-slate-700 dark:text-slate-300 font-medium flex items-center justify-between mb-3">
      ...
      <GlassCard className="p-6 border-sky-400/20">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-sky-400/15">
          <div className="flex items-center gap-2">
            <ListTodo className="w-5 h-5 text-sky-600 dark:text-sky-400" />
            <h3 className="text-base font-serif font-bold text-slate-950 dark:text-white">
              Assigned Site Execution Tasks ({tasks.length})
            </h3>
          </div>
          <span className="text-xs text-slate-700 dark:text-slate-300 font-medium">Quick status updates</span>
        </div>

        {tasks.length === 0 ? (
          <span className="text-xs text-slate-700 dark:text-slate-300 italic font-medium">No tasks currently assigned to you.</span>
        ) : (
              ...
              >
                <div>
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <span className="font-bold text-slate-950 dark:text-white">{task.title}</span>
                    <StatusBadge status={task.status} />
                  </div>
                  <span className="text-[11px] text-slate-700 dark:text-slate-300 font-medium line-clamp-2">{task.description}</span>
                </div>

                <div className="flex items-center justify-between pt-2 border-t border-sky-400/10">
                  <span className="text-[10px] text-sky-600 dark:text-sky-400 font-bold">Priority: {task.priority}</span>
                  <div className="flex items-center gap-1.5">
                    {task.status !== 'COMPLETED' ? (
                      <button
                        type="button"
                        onClick={() => handleTaskStatusChange(task._id, 'COMPLETED')}
                        className="px-2.5 py-1 rounded-lg bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-500/40 text-[10px] font-bold transition-colors"
                      >
                        Mark Done
                      </button>
                    ) : (
